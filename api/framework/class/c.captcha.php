...
class Captcha {
     ...
     */
    function __construct(?string $pKeyValue = null, int $pLength = 4, int $pHeight = 50, ?int $pWidth = null) {
        // Set params
        $this->mLength = min(32, $pLength);
        $this->mHeight = $pHeight;
        $this->mWidth = max($pWidth, $this->mLength * self::CHAR_SIZE + $this->mLength * self::CHAR_SIZE / 10);

        // Set the keys
        $this->SetKeys($pKeyValue);
    ...
    public function ValidateCaptcha(?string $pKeyCode): bool {
        // Check a captcha has not been generated by mistake
        if ($this->mImage)
            return false;

        // Check if captcha exists in session
        if (!Util::GetFromSession($this->mKeyName, __CLASS__))
            return false;

        // Check the case insensitive captcha
        if (!strcasecmp($pKeyCode, $this->mKeyCode)) {
            // Remove key from sesssion
            Util::UnsetFromSession($this->mKeyName, __CLASS__);
            return true;
        } else
            return false;
    }
    ...
    private function SetKeys(?string $pKeyValue) {
        // Set key value
        $this->mKeyValue = $pKeyValue ? $pKeyValue : Crypto::RandomUniqueHash(null, 'md5');

        // Set key name (any deterministic algorithm)
        $this->mKeyName = Crypto::GetHash($this->mKeyValue, true, 'md5');
    }
    ...
    public function GenerateCaptcha() {
        ...
        mt_srand(intval($sec * $usec));

        $char_width = 0;
        $char_widths = array();
        for ($i = 0; $i < strlen($this->mKeyCode); $i++) {
            $char = $this->mKeyCode[$i];

            $pWidth = mt_rand(0, $this->mLength);
            $char_widths[] = $pWidth;
            $char_width += $this->mPNGs[$char]['width'] - $pWidth;
        }

        $offset_x = mt_rand(0, $this->mWidth - $char_width);
        $offset_y = mt_rand(0, $this->mHeight - self::CHAR_SIZE);
        ...
        for ($i = 0; $i < $this->mHeight; $i++) {
            ...
            if ($i > $offset_y && $i < $offset_y + self::CHAR_SIZE) {
                $j = 0;

                for ($k = 0; $k < $offset_x; $k++) {
                    $vImage .= chr(mt_rand(140, 255));
                }

                for ($k = 0; $k < strlen($this->mKeyCode); $k++) {
                    $char = $this->mKeyCode[$k];

                    if (empty($hold_chars[$char])) $hold_chars[$char] = explode("\n", chunk_split(base64_decode($this->mPNGs[$char]['data']), $this->mPNGs[$char]['width'] + 1, "\n"));

                    $vImage .= $this->Randomise(isset($hold_chars[$char][$l]) ? substr($hold_chars[$char][$l], 1) : '', $char_widths[$j]);
                    $j++;
                }

                for ($k = $offset_x + $char_width; $k < $this->mWidth; $k++) {
                    $vImage .= chr(mt_rand(140, 255));
                }

                $l++;
            } else {
                for ($k = 0; $k < $this->mWidth; $k++) {
                    $vImage .= chr(mt_rand(140, 255));
                }
            }
        }
        ...
        // Set image
        $this->mImage = $this->GeneratePNGs($vImage, $this->mWidth, $this->mHeight);

        // Add key to session
        Util::SetInSession($this->mKeyName, $this->mKeyValue, __CLASS__);
    }

    /**
     * Output captcha image on screen
     *
     * @return void
     */
     ...
     * @return string
     */
    public function GetCaptchaBase64(): string {
        return 'data:image/png;base64,' . base64_encode($this->mImage);
    }
    ...
    private function MakeCode() {
        // Process key
        $vCode = $this->mKeyValue;
        $vCode = md5($vCode);
        $vCode = substr($vCode, 0, $this->mLength);

        // case insensitive
        $vCode = strtoupper($vCode);

        // Avoid confusion between 0 (zero) and O (string)
        $vCode = str_replace(0, 'X', $vCode); // O (zero)
        $vCode = str_replace('O', 'Z', $vCode); // O (string)
    ...
    /**
     * Randomise pixels of the image data within certain limits so as to keep it readable. It varies image width too
     *
     * @param string $pScanline
     * @param integer $pWidth
     * @return string
     */
    private function Randomise(string $pScanline, int $pWidth): string {
        $vNewLine = '';
        $vStart = floor($pWidth / 2);
        $vEnd = strlen($pScanline) - ceil($pWidth / 2);

        for ($i = $vStart; $i < $vEnd; $i++) {
            $vPixel = ord($pScanline[intval($i)]);
            if ($vPixel < 190)
                $vNewLine .= chr(mt_rand(0, 205));
            else if ($vPixel > 190)
    ...
    /**
     * Chunk of the given type, data and length adding the relevant CRC
     *
     * @param integer $pLength
     * @param string $pType
     * @param string $pData
     * @return string
     */
    private function ChunkPNG(int $pLength, string $pType, string $pData): string {
        $vRaw = $pType;
        $vRaw .= $pData;
        $vCrc = crc32($vRaw);
        $vRaw .= pack('C4', $vCrc >> 24, $vCrc >> 16, $vCrc >> 8, $vCrc);

        return pack('C4', $pLength >> 24, $pLength >> 16, $pLength >> 8, $pLength) . $vRaw;
    }

    /**
     * Creates greyscale 8bit png
     *
     * @param string $pRawImage
     * @param integer $pWidth
     * @param integer $vHeigth
     * @return string
     */
    private function GeneratePNGs(string $pRawImage, int $pWidth, int $vHeigth): string {
        // SIG
        $vImage = pack('C8', 137, 80, 78, 71, 13, 10, 26, 10);

        // IHDR
        $vRaw = pack('C4', $pWidth >> 24, $pWidth >> 16, $pWidth >> 8, $pWidth);
        $vRaw .= pack('C4', $vHeigth >> 24, $vHeigth >> 16, $vHeigth >> 8, $vHeigth);
        $vRaw .= pack('C5', 8, 0, 0, 0, 0);

        $vImage .= $this->ChunkPNG(13, 'IHDR', $vRaw);

        if (@extension_loaded('zlib')) {
            $pRawImage = gzcompress($pRawImage);
            $vLength = strlen($pRawImage);
        } else {
            // The total length of this image, uncompressed, is just a calculation of pixels
            $vLength = ($pWidth + 1) * $vHeigth;
            ...
            while ($vTempLength > 0) {
                ...
                $substract_value = ($vTempLength < 3800) ? $vTempLength : 3800;
                $vTempLength -= $substract_value;

                while (--$substract_value >= 0) {
                    $s1 += ord($pRawImage[$index]);
                    $s2 += $s1;
            ...
            // This is the same thing as gzcompress($pRawImage, 0) but does not need zlib
            $pRawImage = pack('C3v2', 0x78, 0x01, 0x01, $vLength, ~$vLength) . $pRawImage . $adler_hash;
        ...
        // IDAT
        $vImage .= $this->ChunkPNG($vLength, 'IDAT', $pRawImage);
        // IEND
        $vImage .= $this->ChunkPNG(0, 'IEND', '');

        return $vImage;
    }

    // png image data: each 'data' element is base64_encoded uncompressed IDAT
    // Thanks to DavidMJ for emulating zlib
    private function InitPNGs() {
        $this->mPNGs = array(
            '0' => array(
