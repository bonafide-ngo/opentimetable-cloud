...
class Validate {
    ...
    // Constants
    const string PATTERN_OTC = '/^\d{6}$/';
    const string PATTERN_NAME = '/^(.*){1,64}+$/';
    const string PATTERN_MOBILE = '/^\+[0-9]{7,16}$/';
    const string PATTERN_RESERVED = '/top\s*\-?secret.*chat/i';
    const string PATTERN_SHA1 = '/\b[A-Fa-f0-9]{40}\b/';
    const string PATTERN_SHA256 = '/\b[A-Fa-f0-9]{64}\b/';
    const string PATTERN_SHA512 = '/\b[A-Fa-f0-9]{128}\b/';
    const string PATTERN_LAT = '/^(-?([1-8]?\d(\.\d+)?|90(\.0+)?))$/';
    const string PATTERN_LONG = '/^(-?((1[0-7]\d|[1-9]?\d)(\.\d+)?|180(\.0+)?))$/';
    const string PATTERN_EMAIL = '/^[^@\s]+@[^@\s]+\.[^@\s]+$/'; // https://stackoverflow.com/questions/5601647/html5-email-input-pattern-attribute#answer-36379040

    // Properties
    protected static array $mImageTypes = array(IMG_JPG, IMG_JPEG);
    protected static array $mSettingCodes = array();
    ...
    public static function Mobile(?string $pInput, int $pMaxlength = 16): bool {
        ...
        try {
            ...
            $vNationalPhonelNumber = ''; // Dummy variable passed by reference required by extractCountryCode
